...
export function HopeSection() {
  ...
  const { updateHope, setSheetData } = useSheetStore()

  // 获取当前值和最大值
  const currentHope = typeof safeFormData.hope === 'number' ? safeFormData.hope : 0
  const hopeMax = safeFormData.hopeMax || 6
  ...
  // 获取希望特性
  let hopeTrait = ""
  if (safeFormData.professionRef?.id && safeFormData.cards && Array.isArray(safeFormData.cards)) {
    const professionCard = safeFormData.cards.find(
      (card: StandardCard | null) => card && card.id === safeFormData.professionRef?.id && card.type === "profession"
    ) as StandardCard;
