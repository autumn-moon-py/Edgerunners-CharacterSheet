...
export function HeaderSection({
  ...
  const ancestryInteractionLocked = true

  const isLockedAncestryField = (field: string | null) =>
    field === "ancestry1Ref" || field === "ancestry2Ref"
  ...
  const startEditingName = (field: string, currentValue: string) => {
    if (ancestryInteractionLocked && isLockedAncestryField(field)) {
      return
    }
        ...
        <div className="flex gap-2">
          <div className="flex flex-col">
            ...
            <div className="flex gap-1">
              {editingField === 'ancestry1Ref' && !ancestryInteractionLocked ? (
                <input
                  type="text"
                  value={editingValue}
                  onChange={(e) => setEditingValue(e.target.value)}
                ...
                />
                  ...
                  >
                    <button
                      type="button"
                      disabled={ancestryInteractionLocked}
                      onClick={() => openAncestryModal("ancestry1")}
                      className={`flex-1 text-gray-800 text-xs text-left px-2 py-0.5 focus:outline-none truncate ${ancestryInteractionLocked ? "cursor-default" : "hover:bg-gray-50"}`}
                    >
                      {formData.ancestry1Ref?.name || <span className="print:hidden">选择种族</span>}
                    </button>
                    {formData.ancestry1Ref?.name && !ancestryInteractionLocked && (
                      <button
                        type="button"
              ...
              {editingField === 'ancestry2Ref' && !ancestryInteractionLocked ? (
                <input
                  type="text"
                  value={editingValue}
                  onChange={(e) => setEditingValue(e.target.value)}
                ...
                />
                  ...
                  >
                    ...
                    {formData.ancestry2Ref?.name && !ancestryInteractionLocked && (
                      <button
                        type="button"
