...
import {
  loadBuiltinPackageSource,
  sanitizeBuiltinPackageData,
  saveBuiltinPackageOverride
} from './builtin-package-storage';
...
const CORE_BATCH_RESET_KEY = 'dh_core_batch_reset_version';
const CORE_BATCH_RESET_VERSION = 'edgerunner-core-20260423';
...
export const createStoreActions = (set: SetFunction, get: GetFunction): UnifiedCardActions => ({
  ...
  replaceBuiltinCards: async (importData: ImportData) => {
    ...
    try {
      ...
      await saveBuiltinPackageOverride(rawImportData);
      get()._syncToLocalStorage();
      get()._recomputeAggregations();
      get()._rebuildCardsByType();
      get()._rebuildSubclassIndex();
  ...
  _syncToLocalStorage: () => {
    ...
    try {
      ...
      // builtin 批次会在启动时从默认 JSON/覆盖数据恢复，这里不再重复持久化正文
      localStorage.removeItem(`${STORAGE_KEYS.BATCH_PREFIX}${BUILTIN_BATCH_ID}`);
  ...
  _loadAllCards: async () => {
    ...
    try {
      ...
      // STEP 2: One-time cleanup for legacy custom batches from the previous core pack
      if (!isServer) {
        const appliedResetVersion = localStorage.getItem(CORE_BATCH_RESET_KEY);
        if (appliedResetVersion !== CORE_BATCH_RESET_VERSION) {
          console.log('[UnifiedCardStore] Clearing legacy custom batches for the new core pack...');
          await get().clearAllCustomCards();
          localStorage.setItem(CORE_BATCH_RESET_KEY, CORE_BATCH_RESET_VERSION);
        }
      }
      
      // STEP 3: Load custom cards from localStorage (skip builtin)
      get()._loadCustomCardsFromStorage();
  ...
  _seedBuiltinCards: async () => {
    ...
    try {
      ...
      if (state.batches.has(BUILTIN_BATCH_ID)) {
        console.log('[UnifiedCardStore] Removing existing builtin batch from memory');
        ...
        const newIndex = {
          ...state.index,
          batches: { ...state.index.batches }
        };

        newBatches.delete(BUILTIN_BATCH_ID);
        existingBuiltinBatch?.cardIds.forEach(cardId => {
          newCards.delete(cardId);
        });
        delete newIndex.batches[BUILTIN_BATCH_ID];
        newIndex.totalCards = newCards.size;
        newIndex.totalBatches = newBatches.size;
        newIndex.lastUpdate = new Date().toISOString();
      ...
      console.log(`[UnifiedCardStore] Importing builtin cards from ${builtinPackage.source} source...`);
      await get()._importBuiltinCards(builtinPackage.data, savedDisabledStatus);
  ...
  _importBuiltinCards: async (jsonCardPack: any, previousDisabledStatus?: boolean) => {
    ...
    try {
      const rawImportData = sanitizeBuiltinPackageData({
      ...
      const processedImportData = preprocessVariantFormat(
        JSON.parse(JSON.stringify(rawImportData)) as ImportData
      );

      // Convert import data
      const convertResult = await get()._convertImportData(processedImportData);
      if (!convertResult.success) {
      ...
      const batchInfo: BatchInfo = {
        ...
        cardIds: convertResult.cards.map(card => card.id),
        customFieldDefinitions: processedImportData.customFieldDefinitions ?
          Object.fromEntries(
            Object.entries(processedImportData.customFieldDefinitions)
              .filter(([key, value]) => key !== 'variantTypes' && Array.isArray(value))
          ) as CustomFieldsForBatch : undefined,
        variantTypes: processedImportData.customFieldDefinitions?.variantTypes,
        imageCardIds: imageCardIds.length > 0 ? imageCardIds : undefined,
        imageCount: imageCardIds.length > 0 ? imageCardIds.length : undefined,
        totalImageSize: undefined
      };
