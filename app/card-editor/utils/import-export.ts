...
// 导出卡包
export async function exportCardPackage(data: CardPackageState, format: CardPackageTransferFormat = 'dhcb'): Promise<void> {
  const sanitizedData = sanitizeImportData(data) as CardPackageState
  ...
  if (format === 'dhcb') {
    ...
    try {
      ...
      toast.info('正在导出卡包...');
      const zipBlob = await exportCardPackageWithImages(sanitizedData, sanitizedData.name || '卡包');
      downloadZipFile(zipBlob, sanitizedData.name || '卡包');
    ...
    a.href = url
    a.download = `${sanitizedData.name || '卡包'}.json`
    a.click()
...
// 导入卡包
export function importCardPackage(format: CardPackageTransferFormat = 'dhcb'): Promise<CardPackageState | null> {
  return new Promise((resolve) => {
    const input = document.createElement('input')
    input.type = 'file'
    input.accept = format === 'json' ? '.json' : '.dhcb,.zip'
    input.onchange = async (e) => {
      ...
      try {
        ...
        if (file.name.endsWith('.dhcb') || file.name.endsWith('.zip')) {
          ...
          resolve({
            ...(sanitizeImportData(packageData as ImportData) as CardPackageState),
            isModified: false,
            lastSaved: new Date()
          });
        ...
        const newPackage: CardPackageState = {
          ...(sanitizeImportData(importedData as ImportData) as CardPackageState),
          isModified: false,
          lastSaved: new Date()
        }
        toast.success('卡包已导入')
