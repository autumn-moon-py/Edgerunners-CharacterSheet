...
export async function copyBatchImagesToEditor(cardIds: string[]): Promise<void> {
  if (!isIndexedDBAvailable() || cardIds.length === 0) {
    return;
  }

  const records = await db.images.bulkGet(cardIds);
...
export async function getEditorImagesAsMap(cardIds: string[]): Promise<Map<string, Blob>> {
  ...
  const records = await db.editorImages.bulkGet(cardIds);
  records.forEach(record => {
    if (record) {
      imageMap.set(record.key, record.blob);
    }
  });
