import { createDefaultVariantCard, sanitizeRawVariantCard, type RawVariantCard } from '@/card/variant-card/convert'
...
import { sanitizeCardForPackageType } from '@/card/package-sanitizer'
...
export function createDefaultCard(type: string, packageData: CardPackageState): unknown {
  ...
  switch (type) {
    ...
    case 'variant':
      return createDefaultVariantCard(
        generateRobustCardId(packageData.name || '新建卡包', packageData.author || '作者', 'variant', packageData)
      ) as RawVariantCard
    case 'community':
...
export function copyCard(originalCard: unknown, type: CardType, packageData: CardPackageState): unknown {
  ...
  switch (type) {
    ...
    case 'variant':
      return sanitizeRawVariantCard(copiedCard as Partial<RawVariantCard>)
  ...
  return sanitizeCardForPackageType(type, copiedCard)
}
